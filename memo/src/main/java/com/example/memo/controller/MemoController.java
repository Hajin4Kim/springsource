...
public class MemoController {
  private final MemoService memoService; // TODO: final 과 짝궁 @RequiredArgsConstructor

  // (메모작성 : /memo/create : @get, @post)
  @GetMapping("/create")
  public void getCreateForm(@Valid MemoDto dto, BindingResult result) { // TODO: create.html => th:object="${memoDto}"
    log.info("메모작성 폼 요청");
  }

  @PostMapping("/create")
  public String postCreate(@Valid MemoDto dto, BindingResult result, RedirectAttributes rttr) { // TODO:
                                                                                                // RedirectAttributes
    log.info("메모작성 {}", dto);
  ...
  // (전체메모 : /memo/list : @get)
  @GetMapping("/list")
  public void getList(Model model) { // TODO: ★★★★★★Model★★★★★★ (org.springframework)
    log.info("메모 전체 목록 폼 요청");
    List<MemoDto> list = memoService.list();
    model.addAttribute("list", list); // TODO: 담아서 화면단에 보내주기 Model
  }

  // (메모수정(메모조회 + 수정): /memo/read?mno=1 /memo/update?mno=1 : @get, @post)
  @GetMapping(path = { "/read", "/update" }) // TODO: /read 일때도, /update 일때도 해당 @GetMapping 동작하게끔
  public void getRead(@RequestParam Long mno, Model model) {
    log.info("메모 조회 {}", mno);

    MemoDto dto = memoService.read(mno);
    model.addAttribute("dto", dto); // TODO: 담아서 화면단에 보내주기 Model
  }
