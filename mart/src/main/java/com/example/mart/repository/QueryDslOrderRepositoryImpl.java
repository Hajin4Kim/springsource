...
public class QueryDslOrderRepositoryImpl extends QuerydslRepositorySupport implements QueryDslOrderRepository {
  ...
  @Override
  public List<Object[]> joinTest() { // TODO: 2개 이상 가져오기 => Tuple to Object[]

    // select * from mart_orders mo join mart_member mm on mo.member_member_id =
    // mm.member_id
    QMember qMember = QMember.member;
    QOrder qOrder = QOrder.order; // TODO: new 하는 작업
    ...
    // TODO: SELECT * FROM MART_ORDERS mo JOIN MART_MEMBER mm ON mo.MEMBER_MEMBER_ID
    // = mm.MEMBER_ID LEFT JOIN MART_ORDER_ITEM moi ON moi.ORDER_ORDER_ID =
    // mo.ORDER_ID;
    // TODO: JOIN, Left JOIN으로 3개 가져오기
    JPQLQuery<Tuple> tuple = from(qOrder)
        .join(qMember).on(qOrder.member.eq(qMember))
